...
export default function PromoBanner() {
  ...
  return (
    <div
      className={`relative flex h-full min-h-[320px] flex-col justify-center overflow-hidden rounded-2xl bg-gradient-to-br ${slide.from} ${slide.to} px-8 py-10 text-white transition-all`}
    >
      ...
      <p className="mt-3 max-w-md text-sm text-white/90 sm:text-base">{slide.sub}</p>
      <Link
        href={slide.href}
        className="mt-6 inline-block w-fit rounded-lg bg-white px-5 py-3 text-sm font-bold text-gray-900 transition hover:bg-gray-100"
      >
        {slide.cta} →
      </Link>

      <span className="pointer-events-none absolute -bottom-4 right-4 text-8xl opacity-90 lg:text-9xl" aria-hidden>
        {slide.emoji}
      </span>
